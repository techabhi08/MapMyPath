...
function initialization() {
  ...
  grid[rand(rows / 2 + 1, rows - 1)][rand(columns / 2 + 1, columns - 1)].state =
    "f";
}
...
function draw() {
  ...
  for (var r = 0; r < rows; r++) {
    for (var c = 0; c < columns; c++) {
      if (grid[r][c].state == "w" && grid[r][c].done[0] == "t") continue;
      else {
        if (grid[r][c].state == "w") grid[r][c].done = "true";
        else grid[r][c].done = "false";
      }
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < columns; c++) {
      if (
        ...
        if (
          grid[prex][prey].state == "s" ||
          (grid[prex][prey].state == "f" && (prex != r || prey != c))
        ) {
...
function find_state(state) {
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < columns; c++) {
      if (grid[r][c].state === state) return [r, c];
    }
  }
  return [-1, -1];
}
